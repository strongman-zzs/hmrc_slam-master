...
MapMerger::mapptr MapMerger::MergeMaps(mapptr pMapCurr, mapptr pMapMatch, vector<MapMatchHit> vMatchHits)
{
    ...
    set<ccptr> spHMrC = pMapMatch->GetCCPtrs();
    ...
    if(spHMrC.size() != pMapMatch->msuAssClients.size())
    {
        cout << "\033[1;31m!!! ERROR !!!\033[0m In \"MapMerger::MergeMaps()\": spHMrC.size() != pMapMatch->msuAssClients.size()" << endl;
        ...
        for(set<size_t>::const_iterator sit = pMapMatch->msuAssClients.begin();sit!=pMapMatch->msuAssClients.end();++sit)
            cout << *sit << endl;
        cout << "Associated pCCs:" << endl;
        for(set<ccptr>::const_iterator sit = spHMrC.begin();sit!=spHMrC.end();++sit)
            cout << (*sit)->mClientId << endl;
        throw estd::infrastructure_ex();
    }
...
//                pCC->mpLogger->SetMappingLock(__LINE__,pCC->mClientId);
    ...
    for(set<ccptr>::iterator sit = spHMrC.begin();sit!=spHMrC.end();++sit)
    {
        ccptr pCC = *sit;

        cout << "spHMrC: pCC->mClientId: " << pCC->mClientId << endl;
