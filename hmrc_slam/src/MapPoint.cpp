...
MapPoint::MapPoint(hmrcslam_msgs::MP *pMsg, mapptr pMap, commptr pComm, eSystemState SysState, size_t UniqueId, g2o::Sim3 g2oS_wm_wc)
    : nObs(0),mpReplaced(nullptr),mpMap(pMap),
      mbIsEmpty(false), mbPoseLock(false),mbPoseChanged(false), mbSentOnce(true),mbInOutBuffer(false),
      mLoopPointForKF_LC(defpair), mCorrectedByKF_LC(defpair),mCorrectedReference_LC(defid),
      mLoopPointForKF_MM(defpair), mCorrectedByKF_MM(defpair),mCorrectedReference_MM(defid),
...
void MapPoint::ReduceMessage(hmrcslam_msgs::MP *pMsgFull, hmrcslam_msgs::MPred *pMsgRed)
{
...
void MapPoint::ConvertToMessage(hmrcslam_msgs::Map &msgMap, kfptr pRefKf, g2o::Sim3 mg2oS_wcurmap_wclientmap, bool bForceUpdateMsg)
{
    unique_lock<mutex> lockOut(mMutexOut);

    if((mbSendFull || mSysState == eSystemState::SERVER) && !bForceUpdateMsg)
    {
        hmrcslam_msgs::MP Msg;
        ...
        if(mSysState == eSystemState::SERVER)
        {
            ...
            Converter::CvMatToMsgArrayFixedSize<hmrcslam_msgs::MP::_mPosPred_type,float>(P3D_p,Msg.mPosPred);
        ...
        else
        {
            ...
            Converter::CvMatToMsgArrayFixedSize<hmrcslam_msgs::MP::_mPosPred_type,float>(RefPos,Msg.mPosPred);
            ...
            if(mpRefKF)
            {
                ...
                Converter::CvMatToMsgArrayFixedSize<hmrcslam_msgs::MP::_mPosPar_type,float>(ParPos,Msg.mPosPar);
            }
            else
            {
                Msg.mpParKFId = KFRANGE;
        ...
        Converter::CvMatToMsgArrayFixedSize<hmrcslam_msgs::MP::_mNormalVector_type,float>(mNormalVector,Msg.mNormalVector);

        //Descriptor

        Converter::CvMatToMsgArrayFixedSize<hmrcslam_msgs::MP::_mDescriptor_type,uint8_t>(mDescriptor,Msg.mDescriptor);
...
void MapPoint::UpdateFromMessage(hmrcslam_msgs::MP *pMsg, g2o::Sim3 mg2oS_wcurmap_wclientmap)
{
    //if MP already exists, we reduce it to a Update-Msg to use common interfaces

    hmrcslam_msgs::MPred *pMsgRed = new hmrcslam_msgs::MPred();
...
void MapPoint::UpdateFromMessage(hmrcslam_msgs::MPred *pMsg, g2o::Sim3 mg2oS_wcurmap_wclientmap)
{
    mbOmitSending = true;
...
void MapPoint::WriteMembersFromMessage(hmrcslam_msgs::MP *pMsg, g2o::Sim3 mg2oS_wcurmap_wclientmap)
{
    mbSentOnce=true;
    ...
    Converter::MsgArrayFixedSizeToCvMat<hmrcslam_msgs::MP::_mNormalVector_type,float>(mNormalVector,pMsg->mNormalVector);
    mDescriptor = cv::Mat(1,32,0);
    Converter::MsgArrayFixedSizeToCvMat<hmrcslam_msgs::MP::_mDescriptor_type,uint8_t>(mDescriptor,pMsg->mDescriptor);
...
bool MapPoint::SetPoseFromMessage(hmrcslam_msgs::MP *pMsg, g2o::Sim3 mg2oS_wcurmap_wclientmap)
{
    hmrcslam_msgs::MPred *pMsgRed = new hmrcslam_msgs::MPred();
...
bool MapPoint::SetPoseFromMessage(hmrcslam_msgs::MPred *pMsg, g2o::Sim3 mg2oS_wcurmap_wclientmap)
{
    if(mSysState == eSystemState::CLIENT)
    {
        ...
        if(pPredKf && !pPredKf->isBad())
        {
            ...
            Converter::MsgArrayFixedSizeToCvMat<hmrcslam_msgs::MP::_mPosPred_type,float>(P3D_p,pMsg->mPosPred);
    ...
    else if(mSysState == eSystemState::SERVER)
    {
        ...
        if(pRef)
        {
            Converter::MsgArrayFixedSizeToCvMat<hmrcslam_msgs::MP::_mPosPred_type,float>(P3D_ref,pMsg->mPosPred);
        }
        ...
        if(!pRef)
        {
            ...
            if(pRef)
            {
                Converter::MsgArrayFixedSizeToCvMat<hmrcslam_msgs::MP::_mPosPar_type,float>(P3D_ref,pMsg->mPosPar);
            }
        }
