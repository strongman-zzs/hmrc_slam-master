...
KeyFrame::KeyFrame(hmrcslam_msgs::KF* pMsg, vocptr pVoc, mapptr pMap, dbptr pKFDB, commptr pComm, eSystemState SysState, size_t UniqueId, g2o::Sim3 mg2oS_wcurmap_wclientmap)
    : mFrameId(defpair),mVisId(-1),
      mbFirstConnection(true),mbNotErase(false),
      mbToBeErased(false),
      mpORBvocabulary(pVoc),mpMap(pMap), mpKeyFrameDB(pKFDB),
...
void KeyFrame::ReduceMessage(hmrcslam_msgs::KF *pMsgFull, hmrcslam_msgs::KFred *pMsgRed)
{
...
void KeyFrame::ConvertToMessage(hmrcslam_msgs::Map &msgMap, g2o::Sim3 mg2oS_wcurmap_wclientmap, kfptr pRefKf, bool bForceUpdateMsg)
{
    unique_lock<mutex> lockOut(mMutexOut);

    if((mbSendFull || mSysState == eSystemState::SERVER) && !bForceUpdateMsg)
    {
        hmrcslam_msgs::KF Msg;
        ...
        if(mSysState == eSystemState::SERVER)
        {
            ...
            Converter::CvMatToMsgArrayFixedSize<hmrcslam_msgs::KF::_mTcpred_type,float>(Tcp,Msg.mTcpred);
        ...
        else
        {
            if(this->mId.first == 0)
            {
                Converter::CvMatToMsgArrayFixedSize<hmrcslam_msgs::KF::_mTcpred_type,float>(Tcw,Msg.mTcpred);
            }
            else
            {
                ...
                Converter::CvMatToMsgArrayFixedSize<hmrcslam_msgs::KF::_mTcpred_type,float>(mTcp,Msg.mTcpred);
                ...
                else
                {
                    ...
                    Converter::CvMatToMsgArrayFixedSize<hmrcslam_msgs::KF::_mTcpar_type,float>(mTcp,Msg.mTcpar);
            ...
            Converter::CvMatToMsgArrayFixedSize<hmrcslam_msgs::KF::_mT_SC_type,float>(T_SC,Msg.mT_SC);
        }
        ...
        {
            hmrcslam_msgs::Descriptor MsgDesc;
        ...
        Converter::CvMatToMsgArrayFixedSize<hmrcslam_msgs::KF::_mK_type,float>(mK,Msg.mK);
    ...
    else
    {
        ...
        unique_lock<mutex> lockPose(mMutexPose);
            ...
            if(mbPoseChanged)
            {
                if(this->mId.first == 0)
                {
                    Converter::CvMatToMsgArrayFixedSize<hmrcslam_msgs::KFred::_mTcpred_type,float>(Tcw,Msg.mTcpred);
                }
                else
                {
                    //pose relative to predecessor
                    ...
                    Msg.mpPred_KfId = static_cast<uint16_t>(pPred->mId.first);
                    Msg.mpPred_KfClientId = static_cast<uint8_t>(pPred->mId.second);
                    Converter::CvMatToMsgArrayFixedSize<hmrcslam_msgs::KFred::_mTcpred_type,float>(mTcp,Msg.mTcpred);
...
void KeyFrame::UpdateFromMessage(hmrcslam_msgs::KF *pMsg, g2o::Sim3 mg2oS_wcurmap_wclientmap)
{
    //if KF already exists, we reduce it to an Update-Msg to use common interfaces

    hmrcslam_msgs::KFred *pMsgRed = new hmrcslam_msgs::KFred();
...
void KeyFrame::UpdateFromMessage(hmrcslam_msgs::KFred *pMsg, g2o::Sim3 mg2oS_wcurmap_wclientmap)
{
    mbOmitSending = true;
    ...
    else if(mSysState == eSystemState::SERVER)
    {
        if(!mbPoseLock)
        {
            ...
            else
            {
                ...
                Converter::MsgArrayFixedSizeToCvMat<hmrcslam_msgs::KF::_mTcpred_type,float>(tempTcw,pMsg->mTcpred);
...
void KeyFrame::WriteMembersFromMessage(hmrcslam_msgs::KF *pMsg, g2o::Sim3 mg2oS_wcurmap_wclientmap)
{
    mbSentOnce=true;
    ...
    hmrcslam_msgs::Descriptor TempDesc = pMsg->mDescriptors[0];
    ...
    {
        hmrcslam_msgs::Descriptor DescMsg = pMsg->mDescriptors[idx];
    ...
    mK = cv::Mat(3,3,5);
    Converter::MsgArrayFixedSizeToCvMat<hmrcslam_msgs::KF::_mK_type,float>(mK,pMsg->mK);
    ...
    else if(mSysState == eSystemState::SERVER)
    {
        ...
        Converter::MsgArrayFixedSizeToCvMat<hmrcslam_msgs::KF::_mT_SC_type,float>(T_SC,pMsg->mT_SC);
...
bool KeyFrame::SetPoseFromMessage(hmrcslam_msgs::KF *pMsg, g2o::Sim3 mg2oS_wcurmap_wclientmap)
{
    hmrcslam_msgs::KFred *pMsgRed = new hmrcslam_msgs::KFred();
...
bool KeyFrame::SetPoseFromMessage(hmrcslam_msgs::KFred *pMsg, g2o::Sim3 mg2oS_wcurmap_wclientmap)
{
    if(mSysState == eSystemState::CLIENT)
    {
        // CLIENT
        ...
        else
        {
            ...
            cv::Mat Tcp = cv::Mat(4,4,5);
            Converter::MsgArrayFixedSizeToCvMat<hmrcslam_msgs::KF::_mTcpred_type,float>(Tcp,pMsg->mTcpred);
    ...
    else if(mSysState == eSystemState::SERVER)
    {
        ...
        if(this->mId.first != 0)
        {
            ...
            if(pRef)
            {
                Converter::MsgArrayFixedSizeToCvMat<hmrcslam_msgs::KF::_mTcpred_type,float>(tempTcp,pMsg->mTcpred);
            }
            ...
            if(!pRef)
            {
                ...
                if(pRef)
                {
                    Converter::MsgArrayFixedSizeToCvMat<hmrcslam_msgs::KF::_mTcpar_type,float>(tempTcp,pMsg->mTcpar);
                }
            }
