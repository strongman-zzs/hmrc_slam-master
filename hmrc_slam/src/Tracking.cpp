...
cv::Mat Tracking::GrabImageStereo(const cv::Mat &imLeft, const cv::Mat &imRight) {
    // 步骤 1: 左右图像预处理
    ...
    mpORBextractor->ExtractORB(mImGrayLeft, keypointsLeft, descriptorsLeft);
    mpORBextractor->ExtractORB(mImGrayRight, keypointsRight, descriptorsRight);
    ...
    for (const auto &match : matches) {
        const cv::Point2f &pLeft = keypointsLeft[match.queryIdx].pt;
        const cv::Point2f &pRight = keypointsRight[match.trainIdx].pt;

        float disparity = pLeft.x - pRight.x;
        if (disparity > 0) {
            float depth = mFx * mBaseline / disparity;
            // 创建地图点
            cv::Point3f mapPoint((pLeft.x - mCx) * depth / mFx,
                                 (pLeft.y - mCy) * depth / mFy,
                                 depth);
            // 存储地图点
            mCurrentFrame.mvpMapPoints.push_back(mapPoint);
        }
    }
