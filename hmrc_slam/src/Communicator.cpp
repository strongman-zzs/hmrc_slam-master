      ...
      mbLoadedMap(bLoadedMap)
{
    ...
    if(mpCC->mSysState == eSystemState::CLIENT)
    {
        ...
        mSubMap = mNh.subscribe<hmrcslam_msgs::Map>(MapInTopicName,params::comm::client::miSubMapBufferSize,boost::bind(&Communicator::MapCbClient,this,_1));

        //Publisher
        ss = new stringstream;
        *ss << "MapOut" << SysType << mClientId;
        PubMapTopicName = ss->str();
        mPubMap = mNh.advertise<hmrcslam_msgs::Map>(PubMapTopicName,params::comm::client::miPubMapBufferSize);
    }
    else if(mpCC->mSysState == eSystemState::SERVER)
    {
        ...
        *ss << "MapInTopicName" << mClientId;
        ...
        mSubMap = mNh.subscribe<hmrcslam_msgs::Map>(MapInTopicName,params::comm::server::miSubMapBufferSize,boost::bind(&Communicator::MapCbServer,this,_1));

        //Publisher
        ss = new stringstream;
        *ss << "MapOut" << SysType << mClientId;
        PubMapTopicName = ss->str();
        mPubMap = mNh.advertise<hmrcslam_msgs::Map>(PubMapTopicName,params::comm::server::miPubMapBufferSize);
    }
    else
    {
...
void Communicator::MapCbClient(hmrcslam_msgs::MapConstPtr pMsg)
{
    ...
    if(!pMsg->Keyframes.empty())
    {
        ...
        hmrcslam_msgs::KF msg = pMsg->Keyframes[0];
        ...
        {
            hmrcslam_msgs::KF msgFull = pMsg->Keyframes[idx];
            hmrcslam_msgs::KFred msgRed;
        ...
        {
            hmrcslam_msgs::MP msgFull = pMsg->MapPoints[idx];
            hmrcslam_msgs::MPred msgRed;
            msgRed.mClientId = MAPRANGE;
...
void Communicator::MapCbServer(hmrcslam_msgs::MapConstPtr pMsg)
{
    {
        unique_lock<mutex> lock(mMutexBuffersIn);
        ...
        if(pMsg->KFUpdates.size() > 0)
        {
            ...
            {
                hmrcslam_msgs::KF msgFull;
                msgFull.mClientId = MAPRANGE;
                hmrcslam_msgs::KFred msgRed = pMsg->KFUpdates[idx];
                mlBufKFin.push_back(make_pair(msgFull,msgRed));
            }
        }
        ...
        if(pMsg->MPUpdates.size() > 0)
        {
            ...
            {
                hmrcslam_msgs::MP msgFull;
                msgFull.mClientId = MAPRANGE;
                hmrcslam_msgs::MPred msgRed = pMsg->MPUpdates[idx];
                mlBufMPin.push_back(make_pair(msgFull,msgRed));
            }
        }
...
void Communicator::ProcessKfInClient()
{
    ...
    while (!mlBufKFin.empty() && (ItCount < mKfItBound))
    {
        ...
        {
            hmrcslam_msgs::KF* pMsg = new hmrcslam_msgs::KF();
...
void Communicator::ProcessKfInServer()
{
    ...
    while (!mlBufKFin.empty() && (ItCount < mKfItBound))
    {
        ...
        {
            hmrcslam_msgs::KFred* pMsg = new hmrcslam_msgs::KFred();
...
void Communicator::ProcessMpInClient()
{
    ...
    while (!mlBufMPin.empty() && (ItCount < mMpItBound))
    {
        ...
        if(msgpair.first.mClientId != MAPRANGE)
        {
            hmrcslam_msgs::MP* pMsg = new hmrcslam_msgs::MP();
...
void Communicator::ProcessMpInServer()
{
    ...
    while (!mlBufMPin.empty() && (ItCount < mMpItBound))
    {
        ...
        {
            hmrcslam_msgs::MPred* pMsg = new hmrcslam_msgs::MPred();
